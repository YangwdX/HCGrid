...
/*void read_input_map_hdf5(const char *infile){
    ...
    uint32_t numOfImputPoints = 10000; //竖扫ver19个文件每个文件采样点�?3660*19=69540 , 横扫hor 18*3876=69768
    h_GMaps.data_shape = numOfImputPoints;
    h_GMaps.spec_dim = 2;
    
    //分配内存空间
    h_data = RALLOC(double *, h_GMaps.spec_dim);
    for(int i=0;i<h_GMaps.spec_dim; i++)
        h_data[i] = RALLOC(double, numOfImputPoints);
...
void read_input_coordinate(const char *infile){
    ...
    // Number of input points.
    uint32_t numOfImputPoints = 100000; //竖扫ver19个文件每个文件采样点�?3660*19=69540 , 横扫hor 18*3876=69768
    h_GMaps.data_shape = numOfImputPoints;
    h_GMaps.spec_dim = 4;
...
void read_input_data(const char *infile){
    ...
    uint32_t numOfImputPoints = 10000; //竖扫ver19个文件每个文件采样点�?3660*19=69540 , 横扫hor 18*3876=69768
    // 分配内存空间
    // h_data[dim] = RALLOC(double, numOfImputPoints);
